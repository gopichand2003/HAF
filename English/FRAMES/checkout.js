...
async function handleCheckout(e) {
  ...
  try {
    ...
    // Verify stock availability only for non-custom frames
    for (const item of currentCart) {
      if (!item.isCustomFrame) { // Only check stock for regular frames
    ...
    const options = {
      ...
      handler: async function(response) {
        try {
          ...
          // Update stock only for non-custom frames
          for (const item of currentCart) {
            if (!item.isCustomFrame) {
              const itemRef = doc(db, 'items', item.id.toString());
              const itemDoc = await getDoc(itemRef);
              const currentStock = itemDoc.data().stock;
