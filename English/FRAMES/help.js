  ...
  try {
    ...
    if (unreadMessages.length > 0) {
      ...
      unreadMessages.forEach((msg) => {
        ...
        batchUpdate.update(messageRef, { readByUser: true });
      });
      await batchUpdate.commit();

      // Reset user unread count in metadata
      const chatMetadataRef = doc(db, 'chatMetadata', userId);
      await updateDoc(chatMetadataRef, { userUnreadCount: 0 });
    }
    ...
    .map((msg) => {
      ...
      return `<div class="message ${
        isUser ? 'user' : 'admin'
              ...
              ${attachmentHtml}
              <div class="message-time">${messageTime}${
        isUser ? `<span class="message-status">${statusIcon}</span>` : ''
      }</div>
            </div>`;
    })
    .join('');
...
function setupChatForm() {
  ...
  chatForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      const messagesRef = collection(db, 'users', currentUser.id, 'messages');
      const messageDoc = await addDoc(messagesRef, messageObj);
...
function showErrorMessage(message) {
  ...
  errorElement.style.cssText =
    'padding: 1rem; margin: 1rem 0; background-color: #fee2e2; color: #b91c1c; border-radius: 8px; display: flex; align-items: center; gap: 0.5rem;';
