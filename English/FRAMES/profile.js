...
function showCustomDialog({ title, message, icon, buttons }) {
  ...
  dialogButtons.addEventListener('click', function (e) {
    if (e.target.classList.contains('dialog-btn')) {
      ...
      if (
        typeof buttons.find((btn) => btn.action === action).onClick ===
        'function'
      ) {
...
function loadOrders(orders) {
  ...
  ordersList.innerHTML = sortedOrders
    .map((order) => {
      ...
      const canCancel =
        hoursSinceOrder <= 24 && order.status === 'Confirmed' && !order.shipped;

      // Determine the status to display
      let displayStatus = order.status;
      if (order.shipped && order.status !== 'Cancelled') {
      ...
      return `
            <div class="order-card">
                <div class="order-header">
                    ...
                    <span class="order-date">${orderDate.toLocaleDateString(
                      'en-IN',
                      {
                        year: 'numeric',
                        month: 'long',
                    ...
                    <span class="order-status ${displayStatus.toLowerCase()}">${displayStatus}</span>
                    ${
                      canCancel
                        ? `
                    ...
                    <a href="order-success.html?orderId=${
                      order.orderNumber
      ...
      (address, index) => `
        <div class="address-card ${address.isDefault ? 'default' : ''}">
            ...
            <div class="address-actions">
                <button class="address-btn edit-btn" onclick="editAddress('${
                  address.id
                }')">
                    <i class="fas fa-edit"></i> Edit
                </button>
                <button class="address-btn delete-btn" onclick="deleteAddress('${
                  address.id
                }')">
...
window.closePhoneModal = function () {
  ...
  const messages = document.querySelectorAll(
    '.phone-prompt-message, .phone-success-message'
  );
