    ...
    try {
        ...
        usersSnapshot.forEach(doc => {
            ...
            users.push({
                ...
                joinDate: userData.timestamp?.toDate() || new Date(),
        ...
        document.getElementById('activeUsers').textContent = users.filter(u => u.orderCount > 0).length;
        ...
        if (usersTableBody) {
            usersTableBody.innerHTML = users.map(user => `
                <tr>
                    <td>${user.name}</td>
                    <td>${user.email}</td>
                    <td>${user.joinDate.toLocaleDateString('en-IN')}</td>
