...
async function handleCheckout(e) {
  ...
  try {
    ...
    const order = {
      id: orderNumber,
      orderNumber,
      items: currentCart.map(item => ({...item})), // Create a deep copy of cart items
      address: addressData,
      subtotal,
      tax,
      total,
    ...
    if (paymentMethod === 'cod') {
      await processCODOrder(order, userRef, userData);
    } else {
      await processOnlinePayment(order, userRef, userData);
    }
...
async function processCODOrder(order, userRef, userData) {
    try {
        ...
        // Update the document with the new orders array and empty cart
...
async function processOnlinePayment(order, userRef, userData) {
    const options = {
